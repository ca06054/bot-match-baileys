...
const baileyCleanNumber$1x = (number, full = false) => {
...
const baileyIsValidNumber$1x = (rawNumber) => {
    const regexGroup = /\@g.us\b/gm;
    const exist = rawNumber.match(regexGroup);
    return !exist
};
const baileyIsValidNumber$1 = (rawNumber) => {
    return rawNumber.includes('@s.whatsapp.net') || rawNumber.includes('@g.us');
};
...
class BaileysProvider extends ProviderClass {
    ...
    sendMediax = async (number, imageUrl, text) => {
     ...
     * @returns
     */
    sendTextx = async (number, message) => {
    ...
    sendButtonsx = async (number, text, buttons) => {
        this.emit(
            'notice',
            [
    ...
    sendPollx = async (numberIn, text, poll) => {
    ...
    sendPoll = async (numberIn, text, poll) => {
        const numberClean = baileyCleanNumber(numberIn, true);
    ...
    sendMessagex = async (numberIn, message, { options }) => {
    ...
    sendMessage = async (numberIn, message, { options }) => {
        const number = baileyCleanNumber(numberIn, true);
